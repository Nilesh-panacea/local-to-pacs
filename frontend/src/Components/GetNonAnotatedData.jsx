...
import BasicTable1 from './NonAnotatedDataTable1';
import BasicTable2 from './NonAnotatedDataTable2';

const GetNonAnotatedData = () => {
  const [postData, setPostData] = useState(null); // State to hold the response data
  const [loading, setLoading] = useState(false); // State to indicate loading
  const [nonAnotatedStudyData, setNonAnotatedStudyData] = useState([]);
  const [nonAnotatedDataData, setNonAnotatedDataData] = useState([]);
  ...
  const makePostRequest = async () => {
    ...
    try {
      ...
      setNonAnotatedStudyData(response.data.NonAnotatedStudyData || []);
      setNonAnotatedDataData(response.data.NonAnotatedDataData || []);
    } catch (error) {
    ...
    <div>
      {loading ? (
        ...
        <div>
          ...
          <h2> Table 1</h2>
          <BasicTable1 
          rows = {nonAnotatedStudyData}
            />
        <h2> Table 2</h2>
         <BasicTable2
         rows = {nonAnotatedDataData}
            /> 
        </div>
      ) : (
        <p>No data yet.</p>
